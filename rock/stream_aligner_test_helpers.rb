    ...
    module TestHelpers
        ...
        def stream_aligner_status_reader(task)
            @stream_aligner_status_reader[task] ||=
                syskit_create_reader(task.transformer_stream_aligner_status,
                                     type: :buffer, size: 20)
        end
